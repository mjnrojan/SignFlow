...
export function PropertiesPanel({ activeField, onUpdate, onRemove }: PropertiesPanelProps) {
  if (!activeField) {
    return (
      <div className="w-80 bg-card border-l border-border h-full flex flex-col items-center justify-center p-8 text-center animate-in slide-in-from-right duration-500">
        ...
        <h3 className="text-base font-bold font-['Fraunces'] text-foreground">Select a Field</h3>
        <p className="text-xs text-muted-foreground font-['Syne'] mt-2 leading-relaxed">
          Click on any placed field in the document to configure its properties here.
        </p>
      </div>
    );
  }
    ...
    <div className="w-80 bg-card border-l border-border h-full flex flex-col shrink-0 animate-in slide-in-from-right duration-500 overflow-hidden z-20">
      ...
      <div className="p-6 border-b border-border bg-muted/10">
        <div className="flex items-center justify-between mb-4">
           <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold font-['Fraunces']">Field Properties</h3>
           </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
        ...
        <section className="space-y-4">
           <h4 className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest pl-1 font-['Syne']">Assign To Recipient</h4>
           <div className="space-y-2">
              <button 
                className={`w-full flex items-center gap-3 p-3 rounded-xl border transition-all text-left ${activeField.recipientId === 'R1' ? 'border-primary bg-primary/5 ring-1 ring-primary/20' : 'border-border bg-card'}`}
                onClick={() => onUpdate(activeField.id, { recipientId: 'R1' })}
              >
                 <div className="size-10 rounded-full bg-secondary text-white flex items-center justify-center font-bold text-xs ring-4 ring-muted shadow-sm shrink-0">AS</div>
                 <div className="flex-1 min-w-0">
                    <p className={`text-xs font-bold truncate ${activeField.recipientId === 'R1' ? 'text-primary' : 'text-foreground'}`}>Aarav Sharma</p>
                    <p className="text-[10px] text-muted-foreground truncate">aarav@nepal.np</p>
                 </div>
                 <ChevronRight className={`size-4 ${activeField.recipientId === 'R1' ? 'text-primary' : 'text-border'}`} />
              </button>
              
              <button 
                className={`w-full flex items-center gap-3 p-3 rounded-xl border transition-all text-left ${activeField.recipientId === 'R2' ? 'border-primary bg-primary/5 ring-1 ring-primary/20' : 'border-border bg-card'}`}
                onClick={() => onUpdate(activeField.id, { recipientId: 'R2' })}
              >
                 ...
                 <div className="flex-1 min-w-0">
                    <p className={`text-xs font-bold truncate ${activeField.recipientId === 'R2' ? 'text-primary' : 'text-foreground'}`}>Bikash Tamang</p>
                    <p className="text-[10px] text-muted-foreground truncate">bikash@enterprise.co</p>
                 </div>
                 <ChevronRight className={`size-4 ${activeField.recipientId === 'R2' ? 'text-primary' : 'text-border'}`} />
              </button>
           </div>
        ...
        {/* Validation Options */}
        <section className="space-y-4">
           <h4 className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest pl-1 font-['Syne']">Validation & Options</h4>
           <div className="bg-muted/20 border border-border rounded-2xl p-4 space-y-4">
              <div className="flex items-center justify-between">
                 <div className="flex gap-2 items-center">
                    ...
                    <span className="text-xs font-bold text-foreground">Required Field</span>
                 </div>
      ...
      <div className="p-6 bg-muted/20 border-t border-border mt-auto">
         <button 
           ...
           className="w-full py-4 bg-red-50 hover:bg-red-100 text-red-500 rounded-2xl font-bold text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 group border border-red-200/50"
         >
